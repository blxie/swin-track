    ...
    def __call__(self, worker_id):
        ...
        if worker_info is not None:
            ...
            if self.custom_init_fn is not None:
                self.custom_init_fn(worker_info.dataset, worker_seed,
                                    worker_id)
...
def build_dataloader(dataset,
    ...
    if isinstance(dataset, IterableDataset):
        ...
        if distributed:
            ...
            if do_shuffle:
                event_register.register_epoch_begin_hook(
                    SetEpochHookWrapper(sampler))
        else:
            if do_shuffle:
    ...
    return build_tensor_device_movement_helper(data_loader,
                                               torch.device(device),
