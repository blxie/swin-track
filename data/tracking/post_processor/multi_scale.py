...
class MultiScalePostProcessor:
    ...
    def __call__(self, network_outputs):
        if network_outputs is None:
            return None
        outputs = [
            single_scale_processor(network_output)
            for single_scale_processor, network_output in zip(
                self.single_scale_processors, network_outputs)
        ]
        ...
        return {
            'bbox': bounding_boxes.transpose(0, 1)[torch.arange(N),
                                                   indices, :],
