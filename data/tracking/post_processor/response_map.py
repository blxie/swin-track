...
class ResponseMapTrackingPostProcessing:
    ...
    def __init__(self,
                 enable_gaussian_score_map_penalty,
                 search_feat_size,
                 window_penalty_ratio=None):
        ...
        if enable_gaussian_score_map_penalty:
            self.window = torch.flatten(
                torch.outer(
                    torch.hann_window(search_feat_size[1], periodic=False),
    ...
    def __call__(self, network_output):
        if network_output is None:
            return None
        class_score_map, predicted_bbox = network_output[
            'class_score'], network_output[
        ...
        bounding_box = predicted_bbox[torch.arange(len(predicted_bbox)),
                                      best_idx, :]
