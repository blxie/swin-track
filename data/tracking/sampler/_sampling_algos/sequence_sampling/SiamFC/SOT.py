...
def _get_positive_frame_image_and_bbox(sequence, index):
    ...
    assert bbox_is_valid(frame_bbox) and bounding_box_is_intersect_with_image(
        frame_bbox, frame.get_image_size())
...
def _get_frame_image_and_bbox(sequence, index, rng_engine, reference_bbox):
    ...
    if frame_bbox_validity is not None and not frame_bbox_validity:
        frame_bbox = generate_dummy_bbox_xyxy(frame.get_image_size(),
                                              rng_engine, reference_bbox)
    else:
        assert bbox_is_valid(
            frame_bbox) and bounding_box_is_intersect_with_image(
                frame_bbox, frame.get_image_size())
    assert any(v > 0 for v in frame.get_image_size())
    return frame_image, frame_bbox
...
def _data_getter(sequence, indices, rng_engine: np.random.Generator):
    ...
    x_image, x_bbox = _get_frame_image_and_bbox(sequence, indices[1],
                                                rng_engine, z_bbox)
    return ((z_image, z_bbox), (x_image, x_bbox))


def do_positive_sampling_in_single_object_tracking_dataset_sequence(
    ...
    return _data_getter(
        sequence,
        do_siamfc_pair_sampling_positive_only(
            len(sequence), frame_range,
            sequence.get_all_bounding_box_validity_flag(), sampling_method,
            rng_engine), rng_engine)


def do_negative_sampling_in_single_object_tracking_dataset_sequence(
        sequence: SingleObjectTrackingDatasetSequence_MemoryMapped,
        frame_range: int, rng_engine: np.random.Generator):
    return _data_getter(
        sequence,
        do_siamfc_pair_sampling_negative_only(
            len(sequence), frame_range,
            sequence.get_all_bounding_box_validity_flag(), rng_engine),
        rng_engine)


def do_sampling_in_single_object_tracking_dataset_sequence(
        sequence: SingleObjectTrackingDatasetSequence_MemoryMapped,
        frame_range: int, sampling_method: SiamesePairSamplingMethod,
        rng_engine: np.random.Generator):
    indices, is_positive = do_siamfc_pair_sampling(
        len(sequence), frame_range,
        sequence.get_all_bounding_box_validity_flag(), sampling_method,
        rng_engine)
