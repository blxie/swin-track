...
class _BaseSOTTrackingTripletIterableDatasetSampler:
    ...
    def __init__(
            ...
            aux_frame_range=10,
            sampling_method:
        SiamesePairSamplingMethod = SiamesePairSamplingMethod.interval,
            aux_sampling_method:
        SiamesePairSamplingMethod = SiamesePairSamplingMethod.causal,
            enable_adaptive_frame_range=True,
            negative_sample_random_picking_ratio=0.,
            datasets_sampling_parameters=None,
            datasets_sampling_weight=None,
            data_processor=None):
    ...
    def _pick_random_object_as_negative_sample(
            self, rng_engine: np.random.Generator):
        index_of_dataset = rng_engine.choice(np.arange(len(self.datasets)),
                                             p=self.datasets_sampling_weight)
        ...
        if isinstance(dataset, DetectionDataset_MemoryMapped):
            data = get_one_random_sample_in_detection_dataset_image(
                sequence, rng_engine)
    ...
    def do_sampling(self, rng_engine: np.random.Generator):
        ...
        if self.datasets_sampling_parameters is not None:
            sampling_parameter = self.datasets_sampling_parameters[
                self.current_index_of_dataset]
        ...
        if self.enable_adaptive_frame_range:
            if isinstance(dataset,
                ...
                if sequence.has_fps():
                    ...
                    aux_frame_range = max(int(round(fps / 30 * frame_range)),
                                          1)

        if self.current_is_sampling_positive_sample:
            if isinstance(dataset, DetectionDataset_MemoryMapped):
                z_image, z_bbox = do_sampling_in_detection_dataset_image(
                    sequence, rng_engine)
                data = (z_image, z_bbox, z_image, z_bbox, True, z_image,
                        z_bbox, True, True, True)
                ...
                if len(sampled_data) == 1:
                    data = (sampled_data[0][0], sampled_data[0][0],
                            sampled_data[0][0], sampled_data[0][0], True,
                            sampled_data[0][0], sampled_data[0][0], True, True,
                            True)
                elif len(sampled_data) == 2:
                    data = (sampled_data[0][0], sampled_data[0][0],
                            sampled_data[1][0], sampled_data[1][0], True,
                            sampled_data[0][0], sampled_data[0][0], True,
                            False, True)
                else:
                    data = (sampled_data[0][0], sampled_data[0][0],
                            sampled_data[1][0], sampled_data[1][0], True,
                            sampled_data[2][0], sampled_data[2][0], True,
                            False, False)
            else:
                raise NotImplementedError
        else:
            if isinstance(dataset, DetectionDataset_MemoryMapped):
                z_image, z_bbox = do_sampling_in_detection_dataset_image(
                    sequence, rng_engine)
                x_image, x_bbox = self._pick_random_object_as_negative_sample(
                    rng_engine)
                ...
                if isinstance(dataset,
                              SingleObjectTrackingDataset_MemoryMapped):
                    sampled_data = do_triplet_negative_sampling_in_single_object_tracking_dataset_sequence(
                        sequence, frame_range, aux_frame_range, rng_engine)
                else:
                    sampled_data = do_triplet_negative_sampling_in_multiple_object_tracking_dataset_sequence(
                        sequence, frame_range, aux_frame_range, rng_engine)

                if len(sampled_data) == 1 or (
                        self.negative_sample_random_picking_ratio > 0.
                        and rng_engine.random() <
                        self.negative_sample_random_picking_ratio):
                    x_image, x_bbox = self._pick_random_object_as_negative_sample(
                        rng_engine)
                    aux_image, aux_bbox = self._pick_random_object_as_negative_sample(
                        rng_engine)
                    data = (sampled_data[0][0], sampled_data[0][1], x_image,
                            x_bbox, False, aux_image, aux_bbox, False, False,
                            False)
                else:
                    if len(sampled_data) == 2:
                        aux_image, aux_bbox = self._pick_random_object_as_negative_sample(
                            rng_engine)
                        data = (sampled_data[0][0], sampled_data[0][1],
                                sampled_data[1][0], sampled_data[1][1], False,
                                aux_image, aux_bbox, False, False)
                    else:
                        data = (sampled_data[0][0], sampled_data[0][1],
                                sampled_data[1][0], sampled_data[1][1], False,
                                sampled_data[2][0], sampled_data[2][1], False,
                                False)
            else:
                raise NotImplementedError
...
class SOTTrackingSiameseForwardIteratorDatasetSampler(
        _BaseSOTTrackingTripletIterableDatasetSampler):

    def __init__(
            self,
            forward_only_sequence_picker,
            datasets,
            negative_sample_ratio,
            frame_range=100,
            aux_frame_range=10,
        ...
        SiamesePairSamplingMethod = SiamesePairSamplingMethod.causal,
            ...
            datasets_sampling_weight=None,
            data_processor=None):
        super(SOTTrackingSiameseForwardIteratorDatasetSampler, self).__init__(
            forward_only_sequence_picker, datasets, negative_sample_ratio,
            frame_range, aux_frame_range, sampling_method, aux_sampling_method,
            enable_adaptive_frame_range, negative_sample_random_picking_ratio,
            datasets_sampling_parameters, datasets_sampling_weight,
            data_processor)
...
class SOTTrackingSiameseRandomAccessIteratorDatasetSampler(
        _BaseSOTTrackingTripletIterableDatasetSampler):

    def __init__(
            self,
            random_accessible_sequence_picker,
            datasets,
            negative_sample_ratio,
            frame_range=100,
            aux_frame_range=10,
        ...
        SiamesePairSamplingMethod = SiamesePairSamplingMethod.causal,
            ...
            datasets_sampling_weight=None,
            data_processor=None):
        super(SOTTrackingSiameseRandomAccessIteratorDatasetSampler,
              self).__init__(random_accessible_sequence_picker, datasets,
                             negative_sample_ratio, frame_range,
                             aux_frame_range, sampling_method,
                             aux_sampling_method, enable_adaptive_frame_range,
                             negative_sample_random_picking_ratio,
                             datasets_sampling_parameters,
                             datasets_sampling_weight, data_processor)
