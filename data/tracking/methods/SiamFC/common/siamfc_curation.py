...
def get_jittered_scaling_and_translate_factor(bbox, scaling,
                                              scaling_jitter_factor,
                                              translation_jitter_factor):
    scaling = scaling / torch.exp(torch.randn(2) * scaling_jitter_factor)
    bbox = bbox_xyxy2xywh(bbox)
    max_translate = (torch.tensor(bbox[2:4]) *
                     scaling).sum() * 0.5 * translation_jitter_factor
...
def get_scaling_and_translation_parameters(bbox, area_factor, output_size):
    scaling = get_scaling_factor_from_area_factor(bbox, area_factor,
                                                  output_size)
...
def prepare_SiamFC_curation_with_position_augmentation(
        bbox, area_factor, output_size, scaling_jitter_factor,
        translation_jitter_factor):
    while True:
        scaling = get_scaling_factor_from_area_factor(bbox, area_factor,
                                                      output_size)
        scaling, translate = get_jittered_scaling_and_translate_factor(
            bbox, scaling, scaling_jitter_factor, translation_jitter_factor)

        source_center = bbox_get_center_point(bbox)
        target_center = get_image_center_point(output_size)
        target_center = (torch.tensor(target_center) - translate)

        output_bbox = bbox_scale_and_translate(bbox, scaling, source_center,
                                               target_center)
...
def prepare_SiamFC_curation(bbox, area_factor, output_size):
    curation_scaling, curation_source_center_point, curation_target_center_point = get_scaling_and_translation_parameters(
        bbox, area_factor, output_size)
    output_bbox = bbox_scale_and_translate(bbox, curation_scaling,
                                           curation_source_center_point,
                                           curation_target_center_point)
    output_bbox = torch.tensor(output_bbox)

    curation_parameter = torch.stack(
        (curation_scaling, curation_source_center_point,
         curation_target_center_point))

    return curation_parameter, output_bbox


def do_SiamFC_curation(image,
                       output_size,
                       curation_parameter,
                       interpolation_mode,
                       image_mean=None,
    ...
    output_image, _ = torch_scale_and_translate_half_pixel_offset(
        image, output_size, curation_parameter[0], curation_parameter[1],
        curation_parameter[2], image_mean, interpolation_mode, out_img)
