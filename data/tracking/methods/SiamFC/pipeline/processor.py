...
class SiamTrackerProcessor:
    ...
    def __init__(self, template_size, search_size, template_area_factor,
                 search_area_factor, template_scale_jitter_factor,
                 search_scale_jitter_factor,
                 template_translation_jitter_factor,
                 search_translation_jitter_factor, gray_scale_probability,
        ...
        self.transform = build_SiamTracker_image_augmentation_transformer(
            color_jitter, True)
    ...
    def __call__(self, z_image, z_bbox, x_image, x_bbox, is_positive):
        ...
        z_curated_bbox, z_curation_parameter = SiamTracker_training_prepare_SiamFC_curation(
            z_bbox, self.template_area_factor, self.template_size,
        ...
        x_curated_bbox, x_curation_parameter = SiamTracker_training_prepare_SiamFC_curation(
            x_bbox, self.search_area_factor, self.search_size,
            self.search_scale_jitter_factor,
            self.search_translation_jitter_factor)

        z_curated_image, _ = do_SiamFC_curation(z_image, self.template_size,
                                                z_curation_parameter,
                                                self.interpolation_mode)
        x_curated_image, _ = do_SiamFC_curation(x_image, self.search_size,
                                                x_curation_parameter,
                                                self.interpolation_mode)
...
class SiamFCBatchDataCollator:
    ...
    def __call__(self, data_list):
        ...
        if 'z' in first_element:
            miscellanies_on_host = _collate_dict(data_list, 'z', False,
                                                 miscellanies_on_host)
            miscellanies_on_host = _collate_dict(data_list, 'x', False,
                                                 miscellanies_on_host)
            miscellanies_on_host = _collate_dict(data_list, 'z_bbox', True,
                                                 miscellanies_on_host)
            miscellanies_on_host = _collate_dict(data_list, 'x_bbox', True,
                                                 miscellanies_on_host)

        miscellanies_on_host = _collate_dict(data_list, 'is_positive', True,
                                             miscellanies_on_host)
        if 'z_augmented_image' in first_element:
            miscellanies_on_device = _collate_dict(data_list,
                                                   'z_augmented_image', False,
                                                   miscellanies_on_device)
            miscellanies_on_device = _collate_dict(data_list,
                                                   'x_augmented_image', False,
                                                   miscellanies_on_device)
            miscellanies_on_host = _collate_dict(data_list, 'z_curated_bbox',
                                                 True, miscellanies_on_host)
            miscellanies_on_host = _collate_dict(data_list,
                                                 'z_curation_parameter', True,
                                                 miscellanies_on_host)
            miscellanies_on_host = _collate_dict(data_list, 'x_curated_bbox',
                                                 True, miscellanies_on_host)
            miscellanies_on_host = _collate_dict(data_list,
                                                 'x_curation_parameter', True,
                                                 miscellanies_on_host)
        ...
        labels_batched_on_device = self.label_collator(
            tuple(element['label'] for element in data_list))
