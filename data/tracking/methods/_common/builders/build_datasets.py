    ...
    if "Sampling" in parameters:
        ...
        if "weight" in sampling_parameters:
            for index, dataset_parameters in enumerate(datasets_parameters):
                dataset_parameters["Sampling"]["weight"] = float(
                    sampling_parameters["weight"] * datasets_weight[index]
                )
...
def build_datasets(dataset_config):
    datasets, dataset_parameters = build_dataset_from_config_distributed_awareness(
        dataset_config, _customized_dataset_parameter_handler
    )
