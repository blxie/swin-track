...
def build_sequential_sampling_additional_data_pipelines(
        runtime_vars, branch_config, data_config, runner_config, config,
        event_register: EventRegister, data_source_context,
        capable_for_training):
    ...
    template_feature_cache_service, template_image_mean_cache_service = build_tracking_procedure_template_cache(
        runtime_vars, branch_config, batch_size)
    ...
    if runner_config['type'] == 'default_evaluation':
        assert tracking_evaluation_config[
            'type'] == 'SiamFC' and tracking_data_config['type'] == 'SiamFC'
        curation_parameter_provider = build_curation_parameter_provider(
            tracking_evaluation_config,
            tracking_data_config['pre_processing']['search']['area_factor'])

        host_post_processor = EvaluationHostPipeline(
            batch_size, network_data_config['template_size'],
            network_data_config['search_size'], curation_parameter_provider,
    ...
    if enable_tracking_performance_metrics:
        metric_collector = build_metric_collector(runtime_vars, branch_config,
                                                  data_source_context,
                                                  event_register,
                                                  not capable_for_training)
