...
from .test_only import (
    TestOnlyDatasetTrackingResultSaver,
    Got10kFormatPacker,
    TrackingNetFormatPacker,
)
from .full_annotated import FullyAnnotatedDatasetReportGenerator


# TRACED print test image with bbox
        ...
        for handler_config in self.config:
            if handler_config["type"] == "test_only":
                ...
                if saving_path is not None:
                    ...
                    if format == "got10k":
                        packer = Got10kFormatPacker(
                            tracker_name, datasets,
                            os.path.join(packing_path, "submit"))
                    elif format == "trackingnet":
                        packer = TrackingNetFormatPacker(
                            tracker_name, datasets,
                            os.path.join(packing_path, "submit"))
                    else:
                ...
                collectors.append(
                    TestOnlyDatasetTrackingResultSaver(tracker_name, datasets,
                                                       packing_path, packer))
                ...
                collectors.append(
                    FullyAnnotatedDatasetReportGenerator(
                        tracker_name, datasets, saving_path))
            else:
