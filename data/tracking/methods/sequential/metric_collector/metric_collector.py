...
SequenceEvaluationMetrics = namedtuple(
    ...
    ('average_overlap', 'success_rate_at_iou_0_5', 'success_rate_at_iou_0_75',
...
class EvaluationMetricsCollector:
    ...
    def __init__(self,
                 ...
                 on_the_fly_constructor,
                 multiple_run=False,
                 saving_path: str = None,
                 summary_by_mean=False):
    ...
    def on_epoch_begin(self, epoch):
        ...
        self.sequence_distributor, self.sub_collectors = self.on_the_fly_constructor.construct(
            self.dataset_names, self.saving_path, self.tracker_name,
            constructor_epoch_param)
    ...
    def _do_gathering(self):
        gathering_objects = [
            sub_collector.prepare_gathering()
            for sub_collector in self.sub_collectors
        ]
        ...
        logging_metrics = {}
        for gathered_object, sub_collector in zip(gathered_objects,
                                                  self.sub_collectors):
            if gathered_object is not None:
                logging_metric = sub_collector.accept_gathered(
                    gathered_object, self.task_queue)
        ...
        if len(logging_metrics) > 0:
            ...
            for logging_metric_name in logging_metrics.keys():
                local_metric_definitions.append({
