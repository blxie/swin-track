...
SequenceEvaluationMetrics = namedtuple(
    "SequenceEvaluationMetrics",
    (
        "average_overlap",
        "success_rate_at_iou_0_5",
        "success_rate_at_iou_0_75",
        "success_curve",
        "precision_curve",
...
class FullyAnnotatedDatasetReportGenerator:
    ...
    def __init__(self, tracker_name, datasets: dict, saving_path: str = None):
    ...
    def accept_evaluated_sequence(self, collected_sequences, io_thread):
        if io_thread is not None and self.saving_path is not None and is_main_process(
            ...
            if not self.datasets_evaluation_report_generated:
                io_thread.put(
                    (self._generate_datasets_evaluation_report_header, ))
        ...
        if collected_sequences is not None and len(collected_sequences) > 0:
            for (
                ...
                (
                    ...
                    prec_curve,
                    norm_prec_curve,
                ) = calculate_evaluation_metrics(predicted_bboxes,
                                                 groundtruth_bboxes,
                                                 object_existence)
                ...
                self.cached_sequence_metrics.append((
                    dataset_unique_id,
                    sequence_name,
                    SequenceEvaluationMetrics(
                        ao,
    ...
    def _generate_datasets_evaluation_report_header(self):
        ...
        with open(
                os.path.join(self.saving_path, "performance.csv"),
                "w",
                newline="",
                encoding="utf-8",
        ) as f:
    ...
    def accept_gathered(self, gathered_sequence_metrics, io_thread):
        ...
        for (
        ...
        ) in gathered_sequence_metrics:
            print(
                f"{sequence_name}: success {np.mean(sequence_evaluation_metrics.success_curve):.4f} prec {sequence_evaluation_metrics.precision_curve[20]:.4f} norm prec {sequence_evaluation_metrics.normalized_precision_curve[20]:.4f} "
            )
            ...
            self.run_results[dataset_unique_id][1].append(
                sequence_evaluation_metrics)

            if len(self.run_results[dataset_unique_id][0]) == len(
                ...
                (
                    ...
                    success_rate_at_iou_0_75,
                    fps,
                ) = self._generate_dataset_evaluation_metrics(
                    dataset_unique_id, io_thread,
                    *self.run_results[dataset_unique_id])
                logging_metrics[
                    f"success_score_{dataset_unique_id}"] = success_score
                logging_metrics[
                    f"precision_score_{dataset_unique_id}"] = precision_score
                logging_metrics[
                    f"norm_precision_score_{dataset_unique_id}"] = normalized_precision_score
                logging_metrics[
                    f"average_overlap_{dataset_unique_id}"] = average_overlap
                logging_metrics[
                    f"success_rate_at_iou_0_5_{dataset_unique_id}"] = success_rate_at_iou_0_5
                logging_metrics[
                    f"success_rate_at_iou_0_75_{dataset_unique_id}"] = success_rate_at_iou_0_75
                logging_metrics[f"fps_{dataset_unique_id}"] = fps
            assert len(self.run_results[dataset_unique_id][0]) <= len(
                self.datasets[dataset_unique_id])
        ...
        self.cached_sequence_metrics.clear(
        )  # self.cached_sequence_metrics & gathered_sequence_metrics may be the same object
        return logging_metrics

    def get_summary(self):
        sequence_evaluation_metrics = []
        [
            sequence_evaluation_metrics.extend(sequence[1])
            for sequence in self.run_results.values()
        ]
    ...
    def _save_sequence_predicted_bboxes(
        self,
        dataset_unique_id,
        sequence_name,
        predicted_bboxes,
        ...
        np.save(os.path.join(saving_path, "bounding_box.npy"),
                predicted_bboxes)
        np.savetxt(
            os.path.join(saving_path, "bounding_box.txt"),
            predicted_bboxes,
            fmt="%.3f",
            delimiter=",",
        )
        ...
        np.savetxt(
            os.path.join(saving_path, "time.txt"),
            time_cost_array,
            fmt="%.3f",
            delimiter=",",
        )
        ...
        with open(
                os.path.join(saving_path, "performance.json"),
                "w",
                encoding="utf-8",
                newline="",
        ) as f:
            sequence_report = {
                ...
                "precision_score":
                prec_curve[20],  # center location error @ 20 pix
                "normalized_precision_score": norm_prec_curve[20],
                "fps": 1.0 / np.mean(time_cost_array),
            }
            json.dump(sequence_report, f, indent=2)

    def _generate_dataset_evaluation_report(
        self,
        dataset_unique_id,
        sequence_names: List[str],
        sequence_evaluation_metrics: List[SequenceEvaluationMetrics],
        success_curve,
        precision_curve,
        normalized_precision_curve,
        success_score,
        ...
        with open(
                os.path.join(dataset_report_path, "sequences_performance.csv"),
                "w",
                newline="",
                encoding="utf-8",
        ) as f:
            ...
            for sequence_name, sequence_evaluation_metric in zip(
                    sequence_names, sequence_evaluation_metrics):
                csv_writer.writerow((
                    sequence_name,
                    np.mean(sequence_evaluation_metric.success_curve),
        ...
        draw_success_plot(success_curve[np.newaxis, :], [self.tracker_name],
                          dataset_report_path)
        draw_precision_plot(precision_curve[np.newaxis, :],
                            [self.tracker_name], dataset_report_path)
        draw_normalized_precision_plot(
            normalized_precision_curve[np.newaxis, :],
            [self.tracker_name],
            dataset_report_path,
        )
        ...
        with open(
                os.path.join(dataset_report_path, "performance.json"),
                "w",
                newline="",
                encoding="utf-8",
        ) as f:
    ...
    def _generate_dataset_evaluation_metrics(
        self,
        dataset_unique_id,
        io_thread,
        sequence_names: List[str],
        ...
        if is_main_process():
            if self.saving_path is not None and io_thread is not None:
                io_thread.put((
                    self._generate_dataset_evaluation_report,
                    dataset_unique_id,
                    sequence_names,
                    sequence_evaluation_metrics,
                    success_curve,
    ...
    def _calculate_metrics(
        sequence_evaluation_metrics: List[SequenceEvaluationMetrics], ):
        success_curve = np.mean(
            [
                sequence_evaluation_metric.success_curve
                for sequence_evaluation_metric in sequence_evaluation_metrics
            ],
            axis=0,
        )
        precision_curve = np.mean(
            [
                sequence_evaluation_metric.precision_curve
                for sequence_evaluation_metric in sequence_evaluation_metrics
            ],
            axis=0,
        )
        normalized_precision_curve = np.mean(
            [
                sequence_evaluation_metric.normalized_precision_curve
                for sequence_evaluation_metric in sequence_evaluation_metrics
            ],
            axis=0,
        )
