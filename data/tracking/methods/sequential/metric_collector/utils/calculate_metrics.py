...
def _calc_curves(ious,
    ...
    norm_center_errors = np.asarray(norm_center_errors, np.float64)[:,
                                                                    np.newaxis]

    thr_iou = np.linspace(
        0, 1, parameter.bins_of_intersection_of_union)[np.newaxis, :]
    thr_ce = np.arange(0,
                       parameter.bins_of_center_location_error)[np.newaxis, :]
    thr_nce = np.linspace(
        0, 0.5,
        parameter.bins_of_normalized_center_location_error)[np.newaxis, :]
...
def calculate_evaluation_metrics(
        ...
        groundtruth_bounding_boxes,
        bounding_box_validity_flags,
        parameter: OPEEvaluationParameter = OPEEvaluationParameter):
    ious = bbox_compute_iou_numpy_vectorized(predicted_bounding_boxes,
                                             groundtruth_bounding_boxes)
    assert not (ious > 1.).any()

    groundtruth_bboxes_validity = bbox_is_valid_vectorized(
        groundtruth_bounding_boxes)
    ...
    center_location_errors = calculate_center_location_error_torch_vectorized(
        predicted_bounding_boxes, groundtruth_bounding_boxes, False)
    normalized_center_location_errors = calculate_center_location_error_torch_vectorized(
        predicted_bounding_boxes, groundtruth_bounding_boxes, True)

    predicted_bboxes_validity = bbox_is_valid_vectorized(
        predicted_bounding_boxes)
    ious[~predicted_bboxes_validity] = -1.0
    center_location_errors[~predicted_bboxes_validity] = float('inf')
    normalized_center_location_errors[~predicted_bboxes_validity] = float(
        'inf')

    if bounding_box_validity_flags is not None:
        ious = ious[bounding_box_validity_flags]
        center_location_errors = center_location_errors[
            bounding_box_validity_flags]
    ...
    succ_curve, prec_curve, norm_prec_curve = _calc_curves(
        ious, center_location_errors, normalized_center_location_errors,
        parameter)
    ious[ious < 0.] = 0
