import torch
import torchvision
from data.tracking.methods.SiamFC.common.siamfc_curation import (
    prepare_SiamFC_curation,
    do_SiamFC_curation,
)
...
def _decode_image(image_path):
    # image = np.array(PIL.Image.open(image_path), dtype=np.float32)
    try:
        image = torchvision.io.image.read_image(
        image_path, torchvision.io.image.ImageReadMode.RGB
        )
...
class SequentialWorkerDataPipeline:
    ...
    def __call__(
        ...
        if self.sequence_uuid[index_element] != sequence_uuid:
            ...
            curation_parameter, _ = prepare_SiamFC_curation(
                template_target_bbox,
                self.template_area_factor,
                self.curated_template_image_size,
            )
            curated_first_frame_image, template_image_mean = do_SiamFC_curation(
                template_image,
                self.curated_template_image_size,
                curation_parameter,
        ...
        return (
            ...
            frame_index,
            sequence_length,
            curated_first_frame_image,
            template_image_mean,
            template_target_bbox,
            search_image,
            search_object_existence,
            search_target_bbox,
            first_frame_begin_time,
            search_begin_time,
        )
...
class SequentialWorkerDataProcessor:
    def __init__(
        ...
        self.data_processing_pipeline = SequentialWorkerDataPipeline(
            batch_size,
            template_area_factor,
            curated_template_image_size,
            interpolate_mode,
    ...
    def __call__(self, index, data):
        ...
        (
            ...
            frame_index,
            sequence_length,
            (template_image_path, template_target_bbox),
            (current_search_image_path, current_search_image_target_bbox),
        ) = data
        return self.data_processing_pipeline(
            index,
            sequence_uuid,
            sequence_name,
            sequence_dataset_name,
            ...
            current_search_image_target_bbox,
        )
