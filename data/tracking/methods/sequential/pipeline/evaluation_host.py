...
HostDataProcessorSequenceContext = namedtuple(
    "HostProcessorSequenceContext",
    (
        "name",
        "dataset_name",
        "length",
        "curation_parameter_provider",
        "first_frame_target_bbox",
        "begin_time_recording_array",
        "end_time_recording_array",
    ),
)
HostDataProcessorFrameContext = namedtuple(
    "HostDataProcessorFrameContext",
    ("sequence_uuid", "frame_index", "image_size", "target_existence",
     "target_bbox"),
)
...
class EvaluationHostPipeline:
    ...
    def __init__(
        ...
        self.template_curated_image_feature_cache_service = (
            template_curated_image_feature_cache_service)
        ...
        self.curation_parameter_cache = torch.empty((batch_size, 3, 2),
                                                    dtype=torch.float64)
        self.transform = get_transform()
        self.time_keeping_host_only = time_keeping_host_only

        self.template_curated_image_cache_shape = (
            batch_size,
            3,
            template_curation_image_size[1],
            template_curation_image_size[0],
        )
        self.search_curated_image_cache_shape = (
            batch_size,
            3,
            self.search_curation_image_size[1],
            self.search_curation_image_size[0],
        )
    ...
    def on_device_changed(self, device):
        ...
        if self.template_curated_image_cache is not None:
            self.template_curated_image_cache = self.template_curated_image_cache.to(
                device)
            self.search_curated_image_cache = self.search_curated_image_cache.to(
                device)
    ...
    def on_epoch_begin(self, epoch):
        ...
        self.template_curated_image_cache = torch.empty(
            self.template_curated_image_cache_shape,
            dtype=torch.float,
            device=self.device,
        )
        self.search_curated_image_cache = torch.empty(
            self.search_curated_image_cache_shape,
            dtype=torch.float,
            device=self.device)
        self.template_curated_image_feature_cache_service.create()
        self.template_image_mean_cache_service.create()
        self.tracking_list: Dict[uuid.UUID,
                                 HostDataProcessorSequenceContext] = {}
        self.processing_frame_contexts: List[
            HostDataProcessorFrameContext] = []
    ...
    def pre_initialization(self, samples, targets, miscellanies_on_host,
        ...
        for index_of_object, (
                uuid,
                sequence_name,
                sequence_dataset_name,
                frame_index,
                sequence_length,
                template_curated_image,
                template_object_bbox,
                target_existence,
                target_bounding_box,
                template_image_mean,
                search_image,
                ...
                zip(
                    uuid_list,
                    sequence_name_list,
                    sequence_dataset_name_list,
                    frame_indices_list,
                    sequence_length_list,
                    curated_template_image_list,
                    template_object_bbox_list,
                    target_existing_flags,
            ...
            if frame_index == 1:
                ...
                begin_time_recording_array = torch.empty((sequence_length, ),
                                                         dtype=torch.float64)
                end_time_recording_array = torch.empty((sequence_length, ),
                                                       dtype=torch.float64)

                if self.time_keeping_host_only:
                    first_frame_begin_time = time.perf_counter()
                else:
                    first_frame_begin_time = first_frame_begin_time_batch[
                        index_of_object]
                begin_time_recording_array[0] = first_frame_begin_time

                search_image_curation_parameter_provider = copy.deepcopy(
                    self.search_curation_parameter_provider)
                search_image_curation_parameter_provider.initialize(
                    template_object_bbox)
                assert (template_curated_image is not None
                        and template_image_mean is not None)
                self.template_curated_image_cache[len(self.initializing_uuids),
                                                  ...] = template_curated_image
                self.initializing_uuids.append(uuid)
                self.template_image_mean_cache_service.put(
                    uuid, template_image_mean)

                self.tracking_list[uuid] = HostDataProcessorSequenceContext(
                    sequence_name,
                    sequence_dataset_name,
                    sequence_length,
                ...
                template_image_mean = self.template_image_mean_cache_service.get(
                    uuid)

            sequence_context = self.tracking_list[uuid]
            assert (sequence_context.name == sequence_name
                    and sequence_context.length == sequence_length)

            if self.time_keeping_host_only:
                begin_time = time.perf_counter()
            else:
                begin_time = search_branch_begin_time_batch[index_of_object]
            sequence_context.begin_time_recording_array[
                frame_index] = begin_time

            search_image_size = search_image.shape[1:]
            search_image_size = torch.tensor(
                (search_image_size[1], search_image_size[0]))  # (W, H)
            ...
            search_image_curation_parameter_provider = (
                sequence_context.curation_parameter_provider)

            curation_parameter = search_image_curation_parameter_provider.get(
                self.search_curation_image_size)
            self.curation_parameter_cache[
                index_of_object, :, :] = curation_parameter
            do_SiamFC_curation(
                search_image,
                self.search_curation_image_size,
                curation_parameter,
        ...
        if self.hook is not None:
            self.hook.pre_processing(sequence_contexts,
                                     self.processing_frame_contexts)

        self.cached_x = self.transform(
            self.search_curated_image_cache[:len(uuid_list), ...])
        if len(self.initializing_uuids) == 0:
            return None
        else:
            return self.template_curated_image_cache[:len(self.
                                                          initializing_uuids),
                                                     ...]
    ...
    def on_initialized(self, z_feats):
        ...
        if z_feats is not None:
            self.template_curated_image_feature_cache_service.put_batch(
                self.initializing_uuids, z_feats)
            for initialized_uuid in self.initializing_uuids:
                self.tracking_list[initialized_uuid].end_time_recording_array[
                    0] = time.perf_counter()

        x = self.cached_x
        self.initializing_uuids.clear()
        del self.cached_x
        sequence_uuids = tuple(context.sequence_uuid
                               for context in self.processing_frame_contexts)

        return {
            "z_feat":
            self.template_curated_image_feature_cache_service.get_batch(
                sequence_uuids),
            "x":
            x,
        }
    ...
    def post_tracking(self, outputs):
        ...
        predicted_ious, predicted_bounding_boxes = outputs["conf"], outputs[
            "bbox"]
        assert (len(self.processing_frame_contexts) == len(predicted_ious) ==
        ...
        if self.bounding_box_post_processor is not None:
            predicted_bounding_boxes = self.bounding_box_post_processor(
                predicted_bounding_boxes,
                self.curation_parameter_cache[:predicted_bounding_boxes.
                                              shape[0], ...],
            )
        ...
        for index in range(len(self.processing_frame_contexts)):
            ...
            sequence_context = self.tracking_list[
                sequence_frame_context.sequence_uuid]
            sequence_context.end_time_recording_array[
                sequence_frame_context.frame_index] = time.perf_counter()
            is_last_frame = (sequence_frame_context.frame_index +
                             1 == sequence_context.length)

            if is_last_frame:
                uuid = sequence_frame_context.sequence_uuid
                if self.hook is not None:
                    self.hook.fill_time_cost_array(
                        uuid,
                        sequence_context.begin_time_recording_array,
                        sequence_context.end_time_recording_array,
                    )
            ...
            curation_parameter_provider.update(
                predicted_iou, predicted_bounding_box,
                sequence_frame_context.image_size)
