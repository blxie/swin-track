    ...
    for full_key in list(state_dict.keys()):
        ...
        if key.startswith("layers."):
            ...
            if key[len("layers..") + 1 :].startswith("downsample"):
                ...
                state_dict[
                    f"{prefix}stages.{layer_index + 1}.pre_stage{key_rest}"
                ] = state_dict.pop(full_key)
            else:
                state_dict[full_key.replace("layers", "stages")] = state_dict.pop(
                    full_key
                )
        elif key.startswith("patch_embed"):
            state_dict[
                full_key.replace("patch_embed", "stages.0.pre_stage")
            ] = state_dict.pop(full_key)
    ...
    def forward(self, x, out_indices=None, reshape=True):
        ...
        for i in range(self.num_stages):
            ...
            if i in out_indices:
                ...
                if reshape:
                    x_out = (
                        x_out.view(-1, H, W, self.stage_dims[i])
                        .permute(0, 3, 1, 2)
                        .contiguous()
                    )
                outs.append(x_out)
...
def build_swin_transformer_backbone(
    ...
    transformer = SwinTransformer(
        out_indices=output_layers, frozen_stages=frozen_stages, **params
    )
