...
def build_swin_track_main_components(config, num_epochs, iterations_per_epoch,
    ...
    z_pos_enc, x_pos_enc = build_position_embedding(position_embedding_config,
                                                    z_shape, x_shape,
                                                    transformer_dim)

    with drop_path_allocator:
        encoder = build_encoder(config, drop_path_allocator, transformer_dim,
                                num_heads, mlp_ratio, qkv_bias, drop_rate,
                                attn_drop_rate, z_shape, x_shape)

        decoder = build_decoder(config, drop_path_allocator, transformer_dim,
                                num_heads, mlp_ratio, qkv_bias, drop_rate,
                                attn_drop_rate, z_shape, x_shape)
    ...
    if 'warmup' in drop_path_config and len(drop_path_allocator) > 0:
        if has_training_run:
            ...
            scheduler = build_warmup_scheduler(drop_path_config['warmup'],
                                               drop_path_config['rate'],
...
def build_swin_track(config, load_pretrained, num_epochs, iterations_per_epoch,
