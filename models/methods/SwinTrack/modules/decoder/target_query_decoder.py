...
class TargetQueryDecoderLayer(nn.Module):
    ...
    def forward(self, query, memory, query_pos, memory_pos):
        ...
        query = query + self.drop_path(
            self.self_attn(self.norm_1(query), query_pos, query_pos, None))
        query = query + self.drop_path(
            self.cross_attn(self.norm_2_query(query),
                            self.norm_2_memory(memory), query_pos, memory_pos,
                            None))
...
class TargetQueryDecoderBlock(nn.Module):
    ...
    def __init__(self, num_queries, dim, decoder_layers, z_pos_encoder,
                 x_pos_encoder):
...
def build_target_query_decoder(config, drop_path_allocator, dim, num_heads,
    ...
    z_positional_encoder, x_positional_encoder = build_position_embedding(
        decoder_config['positional_encoding'], z_shape, x_shape, dim)

    decoder = TargetQueryDecoderBlock(num_queries, dim, decoder_layers,
                                      z_positional_encoder,
                                      x_positional_encoder)
    return decoder
