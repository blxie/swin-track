...
def build_model(config: dict, runtime_vars, max_batch_size, num_epochs: int,
                iterations_per_epoch: int, event_register: EventRegister,
    ...
    if config['version'] == 1 and config['type'] == 'SwinTrack':
        ...
        return build_swin_track(config, load_pretrain, num_epochs,
