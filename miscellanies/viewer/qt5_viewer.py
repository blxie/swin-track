...
class _QLabel_auto_scaled_pixmap(QLabel):
    ...
    def redraw_scaled_pixmap(self):
        super(_QLabel_auto_scaled_pixmap,
              self).setPixmap(self._get_scaled_pixmap())
    ...
    def sizeHint(self):
        ...
        if self.unscaled_pixmap is None:
            ...
            h = (float(self.unscaled_pixmap.height()) *
                 w) / self.unscaled_pixmap.width()
        return QSize(w, h)
...
class _CanvasPainter:
    ...
    def draw_label(self, text: str, position: List[int], color: QColor):
        ...
        self.painter.fillRect(
            QRectF(
                position[0],
                position[1] - rendered_object_info_size.height(),
                rendered_object_info_size.width(),
                rendered_object_info_size.height(),
            ),
            QBrush(color),
        )
    ...
    def _list_to_rect(rect: List):
        if all([isinstance(v, int) for v in rect]):
            ...
            return QRectF(float(rect[0]), float(rect[1]), float(rect[2]),
                          float(rect[3]))
...
class _LayoutWidgetCreator:
    ...
    def new_integer_spin_box(self, informative_text, min_value, max_value,
...
def _create_subplots(parent_layout, n_vertical_subplots,
                     n_horizontal_subplots):
...
class Qt5Viewer:
    ...
    def __init__(
        self,
        argv=[],
        n_vertical_subplots=1,
        n_horizontal_subplots=1,
    ):
        ...
        self.subplots = _create_subplots(content_widget_layout,
                                         n_vertical_subplots,
                                         n_horizontal_subplots)
