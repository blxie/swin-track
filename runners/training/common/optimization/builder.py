...
def build_optimization_components(model, runner_config, num_epochs,
    ...
    if 'per_iteration' in lr_scheduler_config:
        ...
        lr_scheduler = build_iter_wise_lr_scheduler(optimizer,
                                                    optimizer_config,
                                                    num_epochs,
        ...
        if lr_scheduler_config['type'] == 'ExponentialLR':
            if 'ultimate_lr' in lr_scheduler_config:
                gamma = math.pow(
                    lr_scheduler_config['ultimate_lr'] /
                    optimizer_config['lr'], 1.0 / num_epochs)
            else:
            ...
            lr_scheduler = torch.optim.lr_scheduler.StepLR(
                optimizer, lr_scheduler_config['lr_drop'])
        else:
            raise NotImplementedError(
                f'Unknown lr_scheduler {lr_scheduler_config["type"]}')
