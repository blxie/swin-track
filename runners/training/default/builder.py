...
def build_default_training_runner(model, runner_config, data_context, config,
                                  event_register: EventRegister):
    num_epochs = config['runs']['num_epochs']

    criterion, loss_composer = build_criteria_and_loss_composer(
        config, runner_config, num_epochs,
        data_context['iterations_per_epoch'], event_register)

    optimizer, lr_scheduler, lr_scheduler_is_per_iteration = build_optimization_components(
        model, runner_config, num_epochs, data_context['iterations_per_epoch'])
    ...
    trainer = DefaultTrainer(criterion, optimizer, lr_scheduler_per_iteration,
