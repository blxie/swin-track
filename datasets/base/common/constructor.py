    ...
    def initialize_from(self, dataset: dict):
        if "context" in dataset:
            ...
            if "bounding_box_format" in dataset_context:
                self.bounding_box_format = BoundingBoxFormat[
                    dataset_context["bounding_box_format"]
                ]
            if "pixel_coordinate_system" in dataset_context:
                self.pixel_coordinate_system = PixelCoordinateSystem[
                    dataset_context["pixel_coordinate_system"]
                ]
            if "pixel_definition" in dataset_context:
                self.pixel_definition = PixelDefinition[
                    dataset_context["pixel_definition"]
                ]
    ...
    def dump_with_default_value(self, dataset: dict):
        ...
        if self.bounding_box_coordinate_system is None:
            if dataset_context["bounding_box_data_type"] == "float":
                ...
                ] = BoundingBoxCoordinateSystem.Spatial.name
            else:
                dataset_context[
                    "bounding_box_coordinate_system"
                ] = BoundingBoxCoordinateSystem.Rasterized.name
        else:
            dataset_context[
                "bounding_box_coordinate_system"
            ] = self.bounding_box_coordinate_system.name

        if self.pixel_coordinate_system is None:
            if dataset_context["bounding_box_data_type"] == "float":
                dataset_context[
                    "pixel_coordinate_system"
                ] = PixelCoordinateSystem.HalfPixelOffset.name
            else:
                dataset_context[
                    "pixel_coordinate_system"
                ] = PixelCoordinateSystem.Aligned.name
        else:
            dataset_context[
                "pixel_coordinate_system"
            ] = self.pixel_coordinate_system.name
...
class BaseDatasetSequenceConstructor:
    def __init__(
        self, sequence: dict, root_path: str, context: _DatasetConstructionContext
    ):
...
class BaseDatasetImageConstructor:
    ...
    def set_category_id(self, category_id):
        assert (
            self.category_id_name_map is not None
        ), "set set_category_id_name_map first"
...
class BaseVideoDatasetConstructor(_BaseDatasetConstructor):
    def __init__(
        ...
        super(BaseVideoDatasetConstructor, self).__init__(
            dataset,
            root_path,
            "video",
            datasets.base.video.dataset.__version__,
            data_version,
            context,
        )
...
class BaseImageDatasetConstructor(_BaseDatasetConstructor):
    def __init__(
        ...
        super(BaseImageDatasetConstructor, self).__init__(
            dataset,
            root_path,
            "image",
            datasets.base.image.dataset.__version__,
            data_version,
            context,
        )
...
class BaseDatasetConstructorGenerator:
    ...
    def __enter__(self):
        ...
        return self.constructor_type(
            self.dataset, self.root_path, self.version, self.context
        )
