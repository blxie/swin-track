from datasets.SOT.dataset import (
...
class SOTDatasetQt5Viewer:
    ...
    def __init__(self, dataset: SingleObjectTrackingDataset_MemoryMapped):
        ...
        if self.dataset.has_category_id_name_map():
            ...
            for category_id in self.dataset.get_category_id_name_map().keys():
                ...
                self.category_id_color_map[category_id] = QColor(
                    color[0], color[1], color[2], int(0.5 * 255))
        else:
        ...
        self.viewer.get_content_region().new_list(
            sequence_names, self._sequence_selected_callback)
    ...
    def _sequence_selected_callback(self, index: int):
        if index < 0:
            return
        self.sequence = SimplePrefetcher(
            DatasetSequenceImageLoader(self.dataset[index]))
        self._stop_timer()
        self._start_timer()
    ...
    def _timer_timeout_callback(self):
        ...
        with canvas.get_painter() as painter:
            draw_object(
                ...
                frame,
                frame,
                self.sequence.iterable.sequence,
                None,
                self.category_id_color_map,
                self.dataset,
                self.dataset,
